...
> [!WARNING]
> **BRZ Garage 目前仍是早期测试版。** 功能、界面、通信协议和本地数据结构仍可能调整；不同年款、市场版本、ECU 软件与 ELM327 适配器尚未全部验证。请在停车状态下完成配置与固件更新，并在升级前保留重要数据。仪表读数和推算结果仅供驾驶参考，不能替代原厂仪表或专业诊断设备。
> 
> 目前只支持MT版本，暂不支持AT版本
> 
> APP开发过程中只在鸿蒙6系统进行过测试，安卓兼容性尚且未知，暂不支持IOS。

## 现有问题
> 转速表和车速表运行时目前仍有一定的卡顿
> 
> APP后台自动唤醒功能仍不稳定，可能导致仪表授时失败
> 
