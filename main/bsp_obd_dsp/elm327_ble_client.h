...
typedef struct {
    ...
    void (*on_parsed_maf)(uint32_t maf_x100);    // mass-air-flow, g/s x100 (PID 01 10)
    void (*on_parsed_fuel_rate)(uint32_t fuel_rate_ml_h); // engine fuel rate, ml/h (PID 01 5E; zero is valid)
    void (*on_parsed_fuel_status)(uint8_t status); // PID 01 03, fuel system 1
    ...
    void (*on_parsed_standard_gear_ratio)(uint16_t ratio_x1000); // SAE J1979 01 A4 reported transmission ratio
