...
#define BRAKE_TEMP_UART_PORT        UART_NUM_1
...
#define BRAKE_TEMP_RETRY_SLOW_MS    5000
...
static query_status_t query_with_cfg(const brake_modbus_cfg_t *cfg, int16_t *temp_x10)
{
    ...
    if (total <= 0) {
        if (s_fail_count == 0 || (s_fail_count % 12) == 0) {
...
static void rs485_temp_task(void *arg)
{
    ...
    if (rs485_has_de_re()) {
        gpio_num_t de_pin = s_de_re_gpio;
        gpio_config_t io_conf = {
            .pin_bit_mask = (de_pin >= 0 && de_pin < 64) ? (1ULL << (unsigned)de_pin) : 0,
    ...
    while (1) {
        ...
        /* An optional sensor may not be installed. After six misses, probe it
         * slowly instead of continuously occupying a UART and the log lock. */
        uint32_t retry_ms = s_fail_count >= BRAKE_TEMP_FAIL_RESET
                          ? BRAKE_TEMP_RETRY_SLOW_MS : BRAKE_TEMP_POLL_MS;
