...
#define ESPNOW_VER              6       // v6: added maf_x100 for fuel-consumption pages
...
#define BROADCAST_IDLE_MS      1000     // no online slave: retain discovery with minimal RF use
...
static void master_task(void *arg) {
    ...
    for (;;) {
        ...
        uint32_t interval_ms = s_linktest_active ? 20 :
                               (espnow_master_online_slaves() ? BROADCAST_INTERVAL_MS : BROADCAST_IDLE_MS);
...
static void apply_packet(const espnow_obd_packet_t *p) {
    ...
    obd_data_set_maf_x100(p->maf_x100);
