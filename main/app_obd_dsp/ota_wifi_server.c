...
#define OTA_RECV_CHUNK           16384     // bounded internal-RAM staging buffer
#define OTA_RECV_CHUNK_PSRAM     16384     // same-size PSRAM fallback
#define OTA_SOCKET_RCVBUF        32768     // keep lwIP bookkeeping bounded
...
#define OTA_IDLE_TIMEOUT_US       (3 * 60 * 1000000LL)  // no phone/update activity: return to normal mode
#define OTA_ERROR_REBOOT_US       (30 * 1000000LL)      // show the error briefly, then restore normal mode
#define OTA_INSTALL_ACK_DELAY_MS  5000      // keep SoftAP alive until final HTTP 200 reaches the phone
#define OTA_INSTALL_STACK_SIZE    6144      // internal stack; flash/cache operations cannot use PSRAM
...
static char s_password[17] = {0};   // fixed bootstrap password; writes still require a random token
...
static volatile bool s_normal_reboot_scheduled = false;
static volatile bool s_install_scheduled = false;
...
/* Verify what is physically stored before changing otadata.  The upload hash
 * protects transport/PSRAM; this second hash also covers the flash write. */
static esp_err_t flash_readback_sha_verify(const esp_partition_t *partition,
                                           size_t image_size,
                                           const uint8_t expected_sha[32])
{
    ...
    while (offset < image_size) {
        ...
        if (chunk > sizeof(s_flash_write_buf)) chunk = sizeof(s_flash_write_buf);
        esp_err_t err = esp_partition_read(partition, offset, s_flash_write_buf, chunk);
        if (err != ESP_OK) {
            mbedtls_sha256_free(&sha);
            return err;
        }
        if (mbedtls_sha256_update(&sha, s_flash_write_buf, chunk) != 0) {
            mbedtls_sha256_free(&sha);
            return ESP_FAIL;
        }
        offset += chunk;
    }
    int sha_err = mbedtls_sha256_finish(&sha, digest);
    mbedtls_sha256_free(&sha);
    if (sha_err != 0 || memcmp(digest, expected_sha, sizeof(digest)) != 0) {
        ESP_LOGE(TAG, "firmware flash read-back SHA-256 mismatch");
        return ESP_ERR_INVALID_CRC;
    }
    ESP_LOGI(TAG, "firmware flash read-back SHA-256 verified");
    return ESP_OK;
}
...
static void reboot_normal_task(void *arg)
{
    (void)arg;
...
// POST /ota/cancel — best-effort fast recovery when the phone gives up.
// A lost phone cannot call this endpoint, so the independent idle watchdog
// remains the final guarantee that OTA mode cannot survive indefinitely.
static esp_err_t cancel_handler(httpd_req_t *req)
{
    if (!validate_token(req)) {
        return send_err(req, "unauthorized");
    }
    if (s_install_scheduled) {
        return send_json_response(req, 200,
                                  "{\"ok\":true,\"message\":\"firmware already installing\"}");
    }
    esp_err_t response = send_json_response(req, 200,
                                             "{\"ok\":true,\"message\":\"rebooting normal mode\"}");
    if (!s_normal_reboot_scheduled) {
        s_normal_reboot_scheduled = true;
        notify_status(OTA_WIFI_STATE_ERROR, "phone cancelled; rebooting", 0, 0);
        xTaskCreate(reboot_normal_task, "ota_normal", 3072, NULL,
                    tskIDLE_PRIORITY + 2, NULL);
    }
    return response;
}

/* Finish firmware installation outside the HTTP server task.  httpd_resp_send()
 * only hands bytes to lwIP; stopping the AP in the request handler can tear down
 * the TCP connection before Android receives the final 200 response.  Returning
 * from the handler first and keeping the AP alive for a short drain interval
 * makes the acknowledgement reliable.  Stopping httpd/WiFi here also releases
 * their internal-RAM stacks and buffer pools before cache-disabled flash work. */
static void firmware_install_task(void *arg)
{
    (void)arg;
    vTaskDelay(pdMS_TO_TICKS(OTA_INSTALL_ACK_DELAY_MS));

    uint32_t expected_size = s_recv.expected_size;
    uint8_t expected_sha[32];
    memcpy(expected_sha, s_recv.expected_sha, sizeof(expected_sha));
    const esp_partition_t *ota_partition = s_recv.ota_partition;
    ...
    if (s_httpd) {
        ...
        if (stop_err != ESP_OK) {
            ESP_LOGW(TAG, "httpd stop before install failed: %s", esp_err_to_name(stop_err));
        }
    }
    esp_err_t wifi_stop_err = esp_wifi_stop();
    if (wifi_stop_err != ESP_OK && wifi_stop_err != ESP_ERR_WIFI_NOT_INIT &&
        wifi_stop_err != ESP_ERR_WIFI_NOT_STARTED) {
        ESP_LOGW(TAG, "WiFi stop before install failed: %s", esp_err_to_name(wifi_stop_err));
    }
    vTaskDelay(pdMS_TO_TICKS(150));
    size_t heap_after = heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    ESP_LOGI(TAG, "OTA transport drained; internal heap %zu -> %zu (+%d B)",
             heap_before, heap_after, (int)(heap_after - heap_before));

    esp_err_t err = ESP_ERR_INVALID_STATE;
    esp_ota_handle_t ota_handle = 0;
    if (ota_partition && s_recv.upload_buf && expected_size > 0) {
        err = esp_ota_begin(ota_partition, expected_size, &ota_handle);
    }
    if (err == ESP_OK) {
    ...
    if (err == ESP_OK) {
        err = flash_readback_sha_verify(ota_partition, expected_size, expected_sha);
    }
    if (err == ESP_OK) {
    ...
    if (err == ESP_OK) {
        ...
        notify_status(OTA_WIFI_STATE_DONE, "firmware updated, rebooting",
                      expected_size, expected_size);
    } else {
        ESP_LOGE(TAG, "firmware install failed after upload: %s; rebooting old firmware",
                 esp_err_to_name(err));
        notify_status(OTA_WIFI_STATE_ERROR, "firmware install failed",
                      expected_size, expected_size);
    }
    recv_reset();
    s_install_scheduled = false;
...
static esp_err_t firmware_handler(httpd_req_t *req)
{
    ...
    if (s_install_scheduled) {
        return send_err(req, "firmware install already scheduled");
    }
    ...
    // Keep the socket window large enough for a 64 KB app chunk without
    // reserving hundreds of KB from the same heap used by WiFi/httpd.
    int rcvbuf_size = OTA_SOCKET_RCVBUF;
    ...
    if (failed) {
        /* Keep every byte that was already acknowledged into PSRAM.  A phone
         * can lose a TCP request while it changes from BLE to the no-internet
         * SoftAP; discarding the whole image here made that harmless radio
         * event fatal.  /ota/status exposes received_size and the next POST may
         * continue at exactly that offset.  Nothing has touched flash yet. */
        notify_status(OTA_WIFI_STATE_RECEIVING, "firmware waiting for resume",
                      s_recv.received_size, expected_size);
        return send_err(req, "receive interrupted; resume from status offset");
    }
    ...
    // The network phase is complete.  Schedule installation on a separate task
    // that waits for this handler to return and for the final TCP response to be
    // acknowledged before it tears down the AP and starts flash work.
    notify_status(OTA_WIFI_STATE_RECEIVING, "firmware installing", expected_size, expected_size);
    s_install_scheduled = true;
    BaseType_t created = xTaskCreate(firmware_install_task, "ota_install",
                                     OTA_INSTALL_STACK_SIZE, NULL,
                                     tskIDLE_PRIORITY + 3, NULL);
    if (created != pdPASS) {
        s_install_scheduled = false;
        recv_reset();
        notify_status(OTA_WIFI_STATE_ERROR, "firmware install task unavailable",
                      expected_size, expected_size);
        return send_err(req, "no memory for firmware install task");
    }
...
static void ota_timeout_task(void *arg)
{
    ...
    for (;;) {
        ...
        bool error_timeout = s_state == OTA_WIFI_STATE_ERROR && idle_us > OTA_ERROR_REBOOT_US;
        bool session_timeout = idle_us > OTA_IDLE_TIMEOUT_US;
        if (error_timeout || session_timeout) {
            ESP_LOGW(TAG, "WiFi OTA %s timeout; rebooting into normal acquisition mode",
                     error_timeout ? "error" : "idle");
            ota_wifi_server_stop();
            /* BLE controller memory may already have been released and the OBD
             * client is intentionally paused.  Stopping only the AP would leave
             * the gauge alive but unable to record.  OTA mode is deliberately
             * non-persistent, so this reboot always returns to normal mode. */
...
bool ota_wifi_server_start(ota_wifi_info_t *info, ota_wifi_status_cb_t callback)
{
    ...
    s_normal_reboot_scheduled = false;
    s_install_scheduled = false;
    ...
    httpd_register_uri_handler(s_httpd, &cancel_uri);
