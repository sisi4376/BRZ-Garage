...
static const vehicle_profile_t s_profiles[] = {
    ...
    {
        // BRZ ZC6 6MT (2013-2020, FA20 NA, Gen1) through the driver's-footwell
        // OBD-II connector. Like the supported ZD8 profile, this keeps the ELM327
        // transport on one serial request/response path and never enters ATMA.
    ...
    {
        // Frozen legacy duplicate retained for source compatibility; it cannot
        // be selected by the product UI, NVS, runtime setter, or phone command.
        // BRZ ZC6 / GT86 ZN6 standard PID path (no ATMA CAN monitoring).
        // Some cheap clone ELM327 adapters do not support ATMA monitoring, or the car's bus does not emit the 0x140/0x360 frames,
        // RPM/coolant temp go through standard OBD PIDs (01 0C / 01 05),
    ...
    },
    {
        ...
        .tire_rolling_radius_m = 0.315f,   // User car / OEM size: 215/40R18
        .gear_count = 6,
    ...
    {
        // BRZ ZD8 6MT through the driver's-footwell OBD-II connector. Gen2 cars
        // gateway/filter the raw powertrain CAN broadcasts at this connector, so
        // repeatedly entering ATMA cannot reliably see 0x241 and only stalls the
        // high-priority RPM/speed queries. Prefer A4 when the ECU really provides
        // a stable ratio, otherwise use synchronized RPM/speed inference.
        .name = "ZD8",
        .final_drive_ratio = 4.100f,
        .tire_rolling_radius_m = 0.315f,   // User car / OEM size: 215/40R18
        .gear_count = 6,
...
{
    if (index >= PROFILE_COUNT) return &s_profiles[VEHICLE_PROFILE_DEFAULT_INDEX];
...
bool vehicle_profile_is_selectable(uint8_t index)
{
    return index == VEHICLE_PROFILE_ZC6_INDEX ||
           index == VEHICLE_PROFILE_ZD8_INDEX;
}

uint8_t vehicle_profile_normalize_index(uint8_t index)
{
    return vehicle_profile_is_selectable(index) ? index :
           VEHICLE_PROFILE_DEFAULT_INDEX;
}

void vehicle_profile_set_active(uint8_t index)
{
    index = vehicle_profile_normalize_index(index);
    s_active_idx = index;
    s_ranges_dirty = true;
...
{
    if (!p) return 0;
    // speed(km/h) = rpm * 0.377 * radius / (gear_ratio * final_drive)
    // calculate_gear divides rpm by speed*constant to obtain
    // gear_ratio*final_drive, so final_drive must not be included here again.
