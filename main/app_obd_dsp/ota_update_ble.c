...
static volatile bool s_wifi_start_pending = false;
...
static void parse_control_write(const uint8_t *buf, uint16_t len)
{
    ...
    if (command == 4) {
        /* Starting the SoftAP initializes WiFi/netif/httpd and can take long
         * enough to starve the Bluedroid GATT callback.  Return from this
         * AUTO_RSP write immediately, then do the radio transition in a
         * one-shot task.  Repeated phone writes are idempotent: they must not
         * try to start a second server or reboot a healthy OTA session. */
        if (!s_wifi_start_pending && ota_wifi_server_get_state() == OTA_WIFI_STATE_IDLE) {
            s_wifi_start_pending = true;
            /* The normal heap routes allocations below 16 KiB to internal
             * DRAM.  At runtime Bluedroid, LVGL and the active OBD link can
             * leave less than one contiguous 6 KiB block there even though
             * the board still has megabytes of PSRAM free.  This used to make
             * an otherwise healthy gauge report "wifi start task unavailable".
             *
             * This one-shot worker does not access cache-disabled flash code;
             * the actual flash writer runs in the OTA server's own tasks.
             * Put only its stack in PSRAM (the TCB remains internal) so OTA
             * entry never competes with the live acquisition tasks for their
             * scarce internal heap. */
            if (xTaskCreateWithCaps(wifi_start_task, "ota_wifi_start", 6144, NULL,
                                    tskIDLE_PRIORITY + 2, NULL,
                                    MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT) != pdPASS) {
                s_wifi_start_pending = false;
                ESP_LOGE(OTA_TAG,
                         "WiFi start worker allocation failed: internal free=%u largest=%u, psram free=%u largest=%u",
                         (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT),
                         (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT),
                         (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT),
                         (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
                notify_status("error", "none", "wifi start task unavailable", 0, 0);
            }
        } else {
            ESP_LOGI(OTA_TAG, "WiFi OTA start already pending/active; duplicate ignored");
        }
    }
}
...
static void start_wifi_ota(void)
{
    /* This function is reachable only through the explicit OTA1/4 control
     * write sent after the user confirms in the non-exported phone service.
     * Merely connecting, syncing or scanning versions never enters OTA. */
    elm327_ble_pause_for_ota();
    espnow_link_pause_for_ota();

    /* BLE and WiFi share both the 2.4 GHz radio and scarce internal DRAM on
     * ESP32-S3.  Keeping Bluedroid alive here made SoftAP startup fail on the
     * physical gauge even though the GATT command had been acknowledged.
     *
     * The Android app treats this disconnect as the expected transport
     * hand-off, scans for OBD-Gauge-OTA-* and obtains the random session token
     * from /ota/discover.  Therefore no BLE notification is needed after this
     * point.  Releasing the controller is intentionally irreversible until the
     * mandatory OTA exit reboot, which always restores normal acquisition. */
    ota_wifi_server_release_bt();

    ota_wifi_info_t info = {0};
    if (!ota_wifi_server_start(&info, NULL)) {
        ESP_LOGE(OTA_TAG, "WiFi OTA start failed after BLE release");
        /* The OBD client and ESP-NOW have been quiesced.  A clean reboot is the
         * only reliable way to restore every radio task after a failed entry. */
...
{
    (void)arg;
    /* Give the automatic GATT write response and Android callback a full
     * connection interval before WiFi coexistence work begins. */
    vTaskDelay(pdMS_TO_TICKS(120));
    start_wifi_ota();
    s_wifi_start_pending = false;
    /* Every successful/failed OTA exit reboots the gauge. Keep this tiny TCB
     * suspended until that reboot instead of self-deleting a WithCaps task,
     * which would itself need to allocate an internal-RAM cleanup task. */
    vTaskSuspend(NULL);
    abort();
}
...
void ota_update_ble_on_disconnect(uint16_t conn_id)
{
    // GATTS receives link events for the outbound ELM327 connection too. Only
    // tear down OTA state when the peer that actually accessed this service left.
    if (!s_connected || conn_id != s_conn_id) return;
    s_connected = false;
    s_conn_id = 0xFFFF;
    s_status_notify_enabled = false;
...
bool ota_update_ble_services_ready(void)
{
...
void ota_update_ble_on_gatts_event(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_CONNECT_EVT:
        // A physical link is not necessarily a phone: the same Bluedroid host
        // also reports our outbound ELM327 link here. Claim it only when this
        // peer accesses an OTA attribute below.
        break;

    case ESP_GATTS_DISCONNECT_EVT:
        ota_update_ble_on_disconnect(param->disconnect.conn_id);
        break;

    case ESP_GATTS_WRITE_EVT:
        if (param->write.handle == s_handle_control ||
            param->write.handle == s_handle_data ||
            param->write.handle == s_handle_status_cccd) {
            ota_update_ble_on_connect(gatts_if, param->write.conn_id);
        }
        if (param->write.handle == s_handle_control) {
    ...
    case ESP_GATTS_READ_EVT:
        if (param->read.handle == s_handle_status) {
            ota_update_ble_on_connect(gatts_if, param->read.conn_id);
            ...
            /* The regular phone app uses the default ATT MTU. Return at most
             * MTU-1 bytes so Android/HarmonyOS can continue with Read Blob
             * requests instead of receiving a truncated oversized response. */
            uint16_t copy_len = available > 22 ? 22 : available;
            if (copy_len > 0) {
