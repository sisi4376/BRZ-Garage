...
// OBD/TIME warning indicators reuse the former "NO SIGNAL" overlay footprint.
static lv_obj_t *s_obd_status_lbl = NULL;
static lv_obj_t *s_time_status_lbl = NULL;
...
static void showroom_load_slot(uint8_t slot)
{
    ...
    if (page == 0) {
        ...
        static const struct { lv_obj_t **scr; void (*init)(void); } pages[] = {
            ...
            { &ui_ScreenPageFuel,       ui_ScreenPageFuel_screen_init },
        };
...
static void boot_enter_default_page(void)
{
    ...
    // Default page: 0=Temp,1=Info,2=Chart,3=Needle,4=Gear+RPM,5=Speed,6=Fuel,7=Trip History,8=Trip Overview
    switch(pg_cfg->default_page) {
        ...
        case 5: target_scr = &ui_ScreenPageSpeed; target_init = ui_ScreenPageSpeed_screen_init; break;
        case 6: target_scr = &ui_ScreenPageFuel;  target_init = ui_ScreenPageFuel_screen_init;  break;
        case 7: target_scr = &ui_ScreenPageTripHistory; target_init = ui_ScreenPageTripHistory_screen_init; break;
        case 8: target_scr = &ui_ScreenPageTripOverview; target_init = ui_ScreenPageTripOverview_screen_init; break;
...
float ui_ext_sweep_tick(bool is_slave, uint8_t configured_brightness)
{
    if (!ui_ext_sweep_active()) {
        ...
        if (s_sweep_bl_last >= 0) {
            ...
            gauge_display_set_brightness(d);
            s_sweep_bl_last = -1;
        }
        return -1.0f;
    }
    ...
    if (bl != s_sweep_bl_last) { gauge_display_set_brightness(bl); s_sweep_bl_last = bl; }
...
/* OBD/TIME indicators occupy the exact former "NO SIGNAL" footprint. */
static lv_obj_t *status_indicator_create(const char *text, lv_coord_t x)
{
    ...
    lv_obj_set_style_text_color(label, lv_color_hex(0xFF4D4D), LV_PART_MAIN);
    lv_obj_set_style_bg_color(label, lv_color_hex(0x000000), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(label, 160, LV_PART_MAIN);
    ...
    lv_obj_align(label, LV_ALIGN_TOP_MID, x, 34);
    lv_obj_add_flag(label, LV_OBJ_FLAG_HIDDEN);
    return label;
}

static void status_indicator_set(lv_obj_t *label, bool show, uint32_t color,
                                 bool *visible, uint32_t *last_color)
{
    if (*last_color != color) {
                                     ...
                                     bool time_synchronized)
{
    ...
    static bool s_time_visible = false;
    static uint32_t s_obd_color = UINT32_MAX;
    static uint32_t s_time_color = UINT32_MAX;
    lv_obj_t *act = lv_scr_act();
    // Keep these fixed top-layer indicators on every driving/trip data page.
    // Settings/scan/boot-animation pages deliberately retain their original layout.
    bool on_gauge_page = (act == ui_ScreenPageTemp || act == ui_ScreenPageInfo ||
                           ...
                           act == ui_ScreenPageSpeed || act == ui_ScreenPageFuel ||
                           act == ui_ScreenPageTripHistory || act == ui_ScreenPageTripOverview ||
                           act == ui_ScreenPageTripIntervals);
     ...
     * phone connected, yellow = synchronized but phone disconnected. */
    uint32_t time_color = !time_synchronized ? 0xFF4D4D :
                          (phone_connected ? 0x45D66F : 0xFFD43B);
    status_indicator_set(s_time_status_lbl, active,
                         time_color,
                         &s_time_visible, &s_time_color);
}
...
void ui_ext_rpm_flash_tick(uint16_t usRpm, bool in_sweep)
{
    ...
    uint16_t warn_thresh = RPM_REDLINE; // yellow line is text-only; flash remains fixed at the 7000 rpm redline
