...
void disp_item_set_text(lv_obj_t *label, disp_item_t item, int32_t value, bool valid)
{
    ...
    if (!valid) {
        ...
        int32_t tenths = ui_round_div_i32(value, 100);
        int32_t magnitude = tenths < 0 ? -tenths : tenths;
        snprintf(text, sizeof(text), "%s%ld.%ld", tenths < 0 ? "-" : "",
        ...
        snprintf(text, sizeof(text), "%ld", (long)ui_round_div_i32(value, 10));
        ...
        int32_t tenths = ui_round_div_i32(value, 10);
        snprintf(text, sizeof(text), "%ld.%ld", (long)(tenths / 10), (long)(tenths % 10));
    } else {
