                                                ...
                                                bool rpm_link_ramp)
{
    ...
    if (scr == ui_ScreenPageTemp || scr == ui_ScreenPageInfo ||
        scr == ui_ScreenPageOilPressure || scr == ui_ScreenPageFuel ||
        scr == ui_ScreenPageTripOverview || scr == ui_ScreenPageTripIntervals ||
        scr == ui_ScreenPageLogo ||
...
static bool ui_screen_updates_live_data(lv_obj_t *scr)
{
    return scr == ui_ScreenPageGear || scr == ui_ScreenPageRpm ||
           ...
           scr == ui_ScreenPageInfo || scr == ui_ScreenPageFuel ||
           scr == ui_ScreenPageTripOverview || scr == ui_ScreenPageTripIntervals;
}
...
void my_timerMain(lv_timer_t * timer)
{
        ...
        while (app_event_recv(&evt)) {
            switch (evt.type) {
                ...
                case APP_EVT_ESPNOW_THRESH_SYNC:
                    ...
                    break;
                case APP_EVT_PHONE_BRIGHTNESS: {
                    /* The BLE host only validates and enqueues this event. Apply
                     * it here, serialized with local settings UI changes, so no
                     * display or NVS operation ever blocks normal GATT traffic. */
                    uint8_t brightness = evt.data.u8;
                    if (brightness >= 10U && brightness <= 100U) {
                        nvs_user_cfg_t cfg = *nvs_cfg_get();
                        cfg.brightness_day = brightness;
                        esp_err_t err = nvs_cfg_set(&cfg);
                        /* nvs_cfg_set updates the runtime configuration before
                         * returning. Apply the requested live value even if an
                         * exceptional flash error prevents persistence. */
                        gauge_display_set_brightness(brightness);
                        if (err == ESP_OK) {
                            ESP_LOGI(TAG, "Phone brightness applied: %u%%", brightness);
                        } else {
                            ESP_LOGW(TAG, "Phone brightness applied, persistence failed: %s",
                                     esp_err_to_name(err));
                        }
                    }
                    break;
                }
                case APP_EVT_PHONE_VEHICLE_PROFILE: {
                    /* Apply phone-originated profile changes from the UI task,
                     * never from the BLE host callback. This keeps GATT writes
                     * constant-time while the active OBD loop switches profile
                     * and resets only its profile-dependent caches. */
                    uint8_t profile = evt.data.u8;
                    if (profile == VEHICLE_PROFILE_ZC6_INDEX ||
                        profile == VEHICLE_PROFILE_ZD8_INDEX) {
                        vehicle_profile_set_active(profile);
                        ESP_LOGI(TAG, "Phone vehicle profile applied: %u (%s)",
                                 profile, vehicle_profile_get_active()->name);
                    }
                    break;
                }
                case APP_EVT_PHONE_ODOMETER_DISPLAY: {
                    /* As with brightness/profile changes, the BLE callback only
                     * validates and enqueues. One user-triggered NVS write is
                     * handled here, away from both phone GATT and OBD tasks. */
                    bool enabled = evt.data.u8 != 0;
                    esp_err_t err = nvs_odometer_set_display(enabled);
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone odometer display: %s", enabled ? "on" : "off");
                    } else {
                        ESP_LOGW(TAG, "Odometer display applied in RAM, persistence failed: %s",
                                 esp_err_to_name(err));
                    }
                    ui_device_odometer_refresh();
                    break;
                }
                case APP_EVT_PHONE_ODOMETER_CALIBRATE: {
                    esp_err_t err = nvs_odometer_calibrate_x10_km(evt.data.u32);
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone odometer calibrated: %lu.%lu km",
                                 (unsigned long)(evt.data.u32 / 10U),
                                 (unsigned long)(evt.data.u32 % 10U));
                    } else {
                        ESP_LOGW(TAG, "Odometer calibrated in RAM, persistence failed: %s",
                                 esp_err_to_name(err));
                    }
                    ui_device_odometer_refresh();
                    break;
                }
                case APP_EVT_PHONE_REFUEL_RESET: {
                    esp_err_t err = nvs_refuel_manual_reset();
                    if (err == ESP_OK) err = nvs_fuel_save();
                    if (err == ESP_OK) ESP_LOGI(TAG, "Phone manually reset since-refuel interval");
                    else ESP_LOGW(TAG, "Refuel interval reset persistence failed: %s",
                                  esp_err_to_name(err));
                    break;
                }
                case APP_EVT_PHONE_REFUEL_DELETE: {
                    esp_err_t err = nvs_refuel_delete_node(evt.data.u32);
                    if (err == ESP_OK) err = nvs_fuel_save();
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone deleted refuel node %lu and merged intervals",
                                 (unsigned long)evt.data.u32);
                    } else {
                        ESP_LOGW(TAG, "Refuel node %lu delete failed: %s",
                                 (unsigned long)evt.data.u32, esp_err_to_name(err));
                    }
                    break;
                }
                case APP_EVT_PHONE_REFUEL_THRESHOLD: {
                    nvs_user_cfg_t cfg = *nvs_cfg_get();
                    cfg.refuel_detect_threshold_ml = (uint16_t)evt.data.u32;
                    esp_err_t err = nvs_cfg_set(&cfg);
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone set automatic refuel threshold: %lu ml",
                                 (unsigned long)evt.data.u32);
                    } else {
                        ESP_LOGW(TAG, "Refuel threshold persistence failed: %s",
                                 esp_err_to_name(err));
                    }
                    break;
                }
                case APP_EVT_PHONE_REFUEL_DISCARD_OLDEST: {
                    esp_err_t err = nvs_refuel_discard_before_first_node(evt.data.u32);
                    if (err == ESP_OK) err = nvs_fuel_save();
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone discarded interval before oldest refuel node %lu",
                                 (unsigned long)evt.data.u32);
                    } else {
                        ESP_LOGW(TAG, "Discard before refuel node %lu failed: %s",
                                 (unsigned long)evt.data.u32, esp_err_to_name(err));
                    }
                    break;
                }
                case APP_EVT_PHONE_CUSTOM_TRIP_BASELINE: {
                    esp_err_t err = nvs_custom_trip_set_baseline(
                        evt.data.trip_baseline.distance_m,
                        evt.data.trip_baseline.duration_s,
                        evt.data.trip_baseline.fuel_ml);
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Phone custom-trip baseline persisted");
                    } else {
                        ESP_LOGW(TAG, "Custom-trip baseline persistence failed: %s",
                                 esp_err_to_name(err));
                    }
                    break;
                }
    ...
    if (!IN_SWEEP && (live_data_screen || rpm_warn_possible || ui_ext_rpm_is_flashing() || ui_ext_rpm_link_ramp_active())) {
        ...
        if (decoded_gear >= 0 && decoded_gear <= GEAR_8) {
            ...
            // Direct and inferred sources are both validated in the data cache.
            // During clutch-in/asynchronous samples, show an honest unknown
            // marker instead of holding or inventing a forward gear.
            eGear = GEAR_NEUTRAL;
            s_gear_unknown = true;
        }
    }
    ...
    if (scr == ui_ScreenPageGear) {
        ...
        static int32_t s_last_combined_rpm = -1;
        static uint8_t s_last_rpm_zone = 0xFF; // 0=normal, 1=yellow, 2=red
        ...
        if ((int32_t)usRpm != s_last_combined_rpm || rpm_zone != s_last_rpm_zone || IN_SWEEP) {
            s_last_combined_rpm = (int32_t)usRpm;
            s_last_rpm_zone = rpm_zone;
    ...
    if (scr == ui_ScreenPageSpeed) {
        ...
        // PID 0D is integer-valued and ECU-filtered. Render the newest sample
        // immediately; an additional UI ramp made the speed gauge visibly lag.
        s_disp_spd = ucSpeed;
        if (s_disp_spd != s_last_spd) {
            s_last_spd = s_disp_spd;
            lv_label_set_text_fmt(ui_SpeedPageArcLabelSpeedText, "%d", (int)s_disp_spd);
            lv_arc_set_value(ui_SpeedPageArcSpeedBack, (uint32_t)s_disp_spd*100/SPEED_DIAL_MAX);
        }
    }

    if (scr == ui_ScreenPageFuel) {
        ui_fuel_page_refresh();
    } else if (scr == ui_ScreenPageTripOverview) {
        ui_trip_overview_refresh();
    } else if (scr == ui_ScreenPageTripIntervals) {
        ui_trip_intervals_refresh();
    } else if (scr == ui_ScreenPageTripHistory) {
    ...
    ui_ota_mode_refresh();
        ...
        if(ucOnlyOnce == 0)
        {
            ...
            if(ulOpenLightTimeCnt > 400 / 200)
            {
                ...
                gauge_display_set_brightness(bright);
...
// The combined Gear+RPM page sits at the front of the carousel: Gear+RPM→Speed→Temp→…
...
void ui_event_oil_pressure_background(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_GESTURE) {
        ...
        if(dir == LV_DIR_RIGHT) {
            ...
            lv_indev_wait_release(lv_indev_get_act());
            _ui_screen_change(&ui_ScreenPageFuel, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0, &ui_ScreenPageFuel_screen_init);
        }
        else if(dir == LV_DIR_BOTTOM) {
...
void ui_event_fuel_background(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_GESTURE) return;
    lv_dir_t dir = lv_indev_get_gesture_dir(lv_indev_get_act());
    if (dir == LV_DIR_RIGHT) {
        lv_indev_wait_release(lv_indev_get_act());
        _ui_screen_change(&ui_ScreenPageOilPressure, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0,
                          &ui_ScreenPageOilPressure_screen_init);
    } else if (dir == LV_DIR_LEFT) {
        lv_indev_wait_release(lv_indev_get_act());
        _ui_screen_change(&ui_ScreenPageTripOverview, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0,
                          &ui_ScreenPageTripOverview_screen_init);
    }
}

void ui_event_trip_overview_background(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_GESTURE) return;
    lv_dir_t dir = lv_indev_get_gesture_dir(lv_indev_get_act());
    if (dir == LV_DIR_RIGHT) {
        lv_indev_wait_release(lv_indev_get_act());
        _ui_screen_change(&ui_ScreenPageFuel, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0,
...
void ui_event_trip_intervals_background(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_GESTURE) return;
    lv_dir_t dir = lv_indev_get_gesture_dir(lv_indev_get_act());
    lv_indev_wait_release(lv_indev_get_act());
...
void ui_event_trip_history_background(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_GESTURE) {
        ...
        if (dir == LV_DIR_RIGHT) {
            ...
            _ui_screen_change(&ui_ScreenPageEasterEgg, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0,
                              &ui_ScreenPageEasterEgg_screen_init);
        } else if (dir == LV_DIR_BOTTOM) {
            ui_trip_history_page(+1);
        } else if (dir == LV_DIR_TOP) {
            ui_trip_history_page(-1);
        }
    }
...
void ui_event_easter_egg_background(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_GESTURE) {
        ...
        if(dir == LV_DIR_RIGHT) {
            lv_indev_wait_release(lv_indev_get_act());
            _ui_screen_change(&ui_ScreenPageTripHistory, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0, &ui_ScreenPageTripHistory_screen_init);
        }
        else if(dir == LV_DIR_LEFT) {
            lv_indev_wait_release(lv_indev_get_act());
            _ui_screen_change(&ui_ScreenPageGear, LV_SCR_LOAD_ANIM_FADE_ON, 5, 0, &ui_ScreenPageGear_screen_init);
...
void ui_init(void)
{
    ...
    // Gear and RPM share one main gauge page; the legacy RPM screen stays compiled
    // for compatibility but is no longer part of the main carousel.
    ...
    ui_ScreenPageTripHistory = NULL;    // completed-trip list lazy-loaded
    ui_ScreenPageTripIntervals = NULL;  // refuel/custom totals lazy-loaded
    ...
    // The other default pages are eagerly created above; Info and Trip History are lazy-loaded.
    if (nvs_cfg_get()->default_page == 1 && ui_ScreenPageInfo == NULL) {
        ui_ScreenPageInfo_screen_init();
    }
    if (nvs_cfg_get()->default_page == 7 && ui_ScreenPageTripHistory == NULL) {
        ui_ScreenPageTripHistory_screen_init();
    }
