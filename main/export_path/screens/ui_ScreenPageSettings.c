...
// 0=TEMP 1=INFO 2=CHART 3=NEEDLE 4=GEAR+RPM 5=SPEED 6=FUEL 7=TRIP HISTORY 8=TRIP OVERVIEW
static const char *page_names = "TEMP\nINFO\nCHART\nNEEDLE\nGEAR + RPM\nSPEED\nFUEL\nTRIP HISTORY\nTRIP OVERVIEW";
#define BOOT_PAGE_COUNT 9
static const char *vehicle_names = "ZC6\nZD8";
static const uint8_t s_vehicle_profile_indices[VEHICLE_PROFILE_SELECTABLE_COUNT] = {
    VEHICLE_PROFILE_ZC6_INDEX,
    VEHICLE_PROFILE_ZD8_INDEX,
};

static uint8_t vehicle_profile_to_roller(uint8_t profile)
{
    return profile == VEHICLE_PROFILE_ZC6_INDEX ? 0U : 1U;
}
...
static lv_obj_t *s_label_trip_gap = NULL;
static uint16_t s_trip_gap_min = TRIP_MERGE_TIMEOUT_LOCKED_MIN;
static const uint16_t s_trip_gap_options[] = {5, 15, 30, 60, 120, 240, 480};
#endif
...
static void on_bright_slider_change(lv_event_t *e)
{
    ...
    nvs_cfg_set(&cfg);
    gauge_display_set_brightness((uint8_t)val);
}
...
#if TRIP_MERGE_TIMEOUT_EDITABLE
static void on_trip_gap_cycle(lv_event_t *e)
{
    (void)e;
    const unsigned option_count = sizeof(s_trip_gap_options) / sizeof(s_trip_gap_options[0]);
    unsigned index = 0;
    for (unsigned i = 0; i < option_count; ++i) {
        if (s_trip_gap_options[i] == s_trip_gap_min) {
            index = i;
            break;
        }
    }
    index = (index + 1U) % option_count;
    s_trip_gap_min = s_trip_gap_options[index];
    lv_label_set_text_fmt(s_label_trip_gap, "%u MIN", s_trip_gap_min);
    nvs_user_cfg_t cfg = *nvs_cfg_get();
    cfg.trip_merge_timeout_min = s_trip_gap_min;
    nvs_cfg_set(&cfg);
}
#endif
...
{
    uint8_t roller_index = (uint8_t)lv_roller_get_selected(s_roller_vehicle);
    if (roller_index >= VEHICLE_PROFILE_SELECTABLE_COUNT) roller_index = 1U;
    // Apply immediately; the central setter also persists the selected profile.
    vehicle_profile_set_active(s_vehicle_profile_indices[roller_index]);
}
...
void ui_ScreenPageSettings_screen_init(void)
{
    ...
    lv_roller_set_selected(s_roller_vehicle,
                           vehicle_profile_to_roller(cfg->vehicle_profile_idx),
                           LV_ANIM_OFF);
    ...
    lv_obj_align(label_bright, LV_ALIGN_CENTER, 0, 44);
    ...
    lv_obj_align(s_slider_bright, LV_ALIGN_CENTER, 0, 63);
    ...
    lv_obj_align(s_label_bright_val, LV_ALIGN_CENTER, 0, 82);

    // ====== Row 5: Same-trip power-off window ======
    lv_obj_t *label_trip_gap = lv_label_create(ui_ScreenPageSettings);
    lv_label_set_text(label_trip_gap, "TRIP GAP");
    lv_obj_set_style_text_font(label_trip_gap, &ui_font_FontTypoderSize16, LV_PART_MAIN);
    lv_obj_set_style_text_color(label_trip_gap, ui_theme_color_lv(UI_COLOR_TEXT_SECONDARY), LV_PART_MAIN);
    lv_obj_align(label_trip_gap, LV_ALIGN_CENTER, -55, 106);

    lv_obj_t *btn_trip_gap = lv_btn_create(ui_ScreenPageSettings);
    lv_obj_set_style_clip_corner(btn_trip_gap, true, 0);
    lv_obj_set_size(btn_trip_gap, 96, 26);
    lv_obj_align(btn_trip_gap, LV_ALIGN_CENTER, 68, 106);
    lv_obj_set_style_bg_color(btn_trip_gap, lv_color_hex(0x333333), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(btn_trip_gap, 255, LV_PART_MAIN);
    lv_obj_set_style_radius(btn_trip_gap, 13, LV_PART_MAIN);
    lv_obj_set_style_pad_all(btn_trip_gap, 2, LV_PART_MAIN);
    lv_obj_t *label_trip_gap_value = lv_label_create(btn_trip_gap);
#if TRIP_MERGE_TIMEOUT_EDITABLE
    s_trip_gap_min = cfg->trip_merge_timeout_min;
    s_label_trip_gap = label_trip_gap_value;
    lv_label_set_text_fmt(label_trip_gap_value, "%u MIN", s_trip_gap_min);
    lv_obj_add_event_cb(btn_trip_gap, on_trip_gap_cycle, LV_EVENT_CLICKED, NULL);
#else
    lv_label_set_text_fmt(label_trip_gap_value, "%u MIN", TRIP_MERGE_TIMEOUT_LOCKED_MIN);
    /* Keep the row visible, but deliberately disable interaction while locked. */
    lv_obj_add_state(btn_trip_gap, LV_STATE_DISABLED);
#endif
    lv_obj_set_style_text_font(label_trip_gap_value, &ui_font_FontTypoderSize16, LV_PART_MAIN);
    lv_obj_set_style_text_color(label_trip_gap_value, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_obj_center(label_trip_gap_value);
    ...
    lv_obj_align(label_rc, LV_ALIGN_CENTER, -40, 136);   // label + button share one row to save height
    ...
    lv_obj_align(s_btn_rc, LV_ALIGN_CENTER, 70, 136);
