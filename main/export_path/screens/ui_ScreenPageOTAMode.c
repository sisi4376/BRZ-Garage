...
void ui_ScreenPageOTAMode_screen_init(void)
{
    ...
    // Human-readable firmware version. The build identity is available in
    // the device manifest and on the Device Info page.
    ui_LabelOTAModeVersion = lv_label_create(ui_ScreenPageOTAMode);
    lv_label_set_text_fmt(ui_LabelOTAModeVersion, "FW %s", OBD_GAUGE_FIRMWARE_VERSION);
