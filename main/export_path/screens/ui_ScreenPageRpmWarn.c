...
static void on_rpm_warn_slider_change(lv_event_t *e)
{
    ...
    // Configurable yellow line, step 500: 1000~6500. Redline stays fixed at 7000.
    val = ((val + 250) / 500) * 500;
    if (val < 1000) val = 1000;
    if (val > 6500) val = 6500;
    lv_slider_set_value(s_slider_rpm_warn, val, LV_ANIM_OFF);
    lv_label_set_text_fmt(s_label_rpm_warn_val, "%d", (int)val);
...
void ui_ScreenPageRpmWarn_screen_init(void)
{
    ...
    lv_label_set_text(title, "YELLOWLINE RPM");
    ...
    lv_label_set_text(sub, "RPM turns yellow above this limit");
    ...
    lv_label_set_text(range, "Y 1000-6500   RED 7000   MAX 9000");
