...
class CalendarBookkeepingAndMultiPhoneTests(unittest.TestCase):
    def test_driving_calendar_and_total_overview_are_present(self):
        ...
        self.assertIn('card(body, "驾驶日历")', main)
        ...
        self.assertIn("today.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY))", calendar)
        self.assertIn("date.isAfter(visibleEnd)", calendar)
        self.assertIn("fun colorForDistance(distanceM: Long)", calendar)
        self.assertIn("distanceM < 100_000L", calendar)
        self.assertNotIn("maxDistance", calendar)
        self.assertIn("100 km 及以上为最高强度", main)
        self.assertIn("DrivingCalendarView.ACTIVITY_COLORS", main)
        self.assertIn('bookkeepingMetric("累计里程"', main)
        self.assertIn("refreshRevisionToggle", main)
        self.assertIn('label("历史驾驶记录"', main)
        self.assertIn("list.addHeaderView(body, null, false)", main)
        self.assertIn("historyEmptyText.visibility = if (trips.isEmpty())", main)
    ...
    def test_bookkeeping_has_swipe_sections_and_visual_statistics(self):
        ...
        self.assertIn("primaryBookkeepingAction", main)
        self.assertIn("bookkeepingMetric", main)
        self.assertIn("bookkeepingEmpty", main)
        self.assertIn("content.animate().alpha(1f)", main)
        self.assertIn("CREATE TABLE expenses", database)
        self.assertIn("item.maintenance", chart)
        self.assertIn("item.daily", chart)
        self.assertIn("repeat(4)", chart)
        self.assertIn("暂无月度开销", chart)
