...
- Protocol path: BLE + ELM327; the exposed ZC6/ZD8 profiles both use one serial OBD/PID request-response path
- Exposed vehicle profiles: ZC6 and ZD8; legacy profiles remain frozen in source only
- Multi-gauge: one master + multiple slaves linked over ESP-NOW
- Validation status: ZD8 6MT is road-adapted; ZC6 6MT still requires road validation

## Features

- BLE scan and connection for ELM327-compatible adapters
- **Single-path ZC6/ZD8 polling**: neither exposed profile enters ATMA, avoiding ELM327 switching delays between OBD requests and CAN monitoring
- **ZC6 oil temperature**: retains the FA20-specific Toyota Mode 21 request while other common values use OBD PIDs
...
- User configuration and fuel-trip records are persisted in NVS; legacy generic mileage statistics remain runtime-only. Full trip records can be resumably synchronized to the Android companion over BLE.
