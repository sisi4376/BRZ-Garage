...
OBD BRZ Gauge 是一个基于 ESP-IDF 的车载圆形仪表显示项目。本分支默认运行在微雪 Waveshare ESP32-S3-Touch-AMOLED-1.75-B（466×466 CO5300/CST9217）上，并保留原 ESP32-S3-Touch-LCD-1.85 选项。项目通过 BLE 连接兼容 ELM327 的 OBD 适配器读取车辆数据；开发阶段还可通过 TCP ELM327 模拟器和浏览器免烧录预览进行联调。

## 当前状态

- 默认硬件平台：微雪 Waveshare ESP32-S3-Touch-AMOLED-1.75-B；原 1.85" LCD 可在 menuconfig 切换
- 软件栈：ESP-IDF 5.5+、LVGL 8、微雪官方 AMOLED BSP 3.x
- 通信链路：BLE + ELM327；当前开放的 ZC6/ZD8 均使用单路 OBD/PID 请求响应
- 当前开放车型：ZC6、ZD8；其他历史 profile 仅保留在源码中并已冻结
- 三连表：一主多从，通过 ESP-NOW 联动
- 当前验证状态：ZD8 6MT 已完成实车适配；ZC6 6MT 尚待实车验证

## 功能概览

- 支持扫描并连接兼容 ELM327 的 BLE OBD 设备
- **ZC6/ZD8 单路轮询**：两个开放车型均不进入 ATMA，避免 ELM327 在 OBD 请求与 CAN 监听间切换造成延迟
- **ZC6 机油温度**：保持 FA20 专用的 Toyota Mode 21 请求，其余常用数据通过 OBD PID 获取
...
- 用户配置和油耗行程通过 NVS 持久化；旧的通用里程统计仍只在运行时累计。完整行程可通过 BLE 断点同步到 Android 手机。
...
浏览器访问 <http://127.0.0.1:8080>。连接 ELM327-emulator 的完整步骤见 [ELM327_EMULATOR.md](ELM327_EMULATOR.md)，新开发板移植状态见 [AMOLED_175_PORT.md](AMOLED_175_PORT.md)。
