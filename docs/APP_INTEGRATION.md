...
## Automatic phone time sync / 手机 BLE 自动授时

The same Device Info service exposes a write-only time characteristic:

- Service UUID: `0x1FFA`
- Characteristic UUID: `0x0002`
- Payload: unsigned 64-bit Unix epoch **seconds**, little-endian (exactly 8 bytes)
- Accepted range: `2024-01-01T00:00:00Z` through `2100-01-01T00:00:00Z`

The companion app should scan for the gauge after vehicle power-on, connect,
write the phone's current Unix time once with a response, synchronize pending
trip history, and then disconnect. When
RaceChrono is disabled on a standalone gauge, the firmware advertises this
service for 120 seconds after boot. The ELM327 remains on its independent GATT
client connection.
...
- 服务 UUID：`0x1FFA`
- 特征 UUID：`0x0002`
- 载荷：无符号 64 位 Unix **秒**，小端序，固定 8 字节
- 有效范围：`2024-01-01T00:00:00Z` 至 `2100-01-01T00:00:00Z`

车辆上电后，App 应在后台扫描仪表，连接后写入一次手机当前 Unix
时间（使用带响应写入），同步待确认行程后再断开。独立仪表关闭 RaceChrono 时，
固件会在启动后的 120 秒内广播该服务；ELM327 继续使用另一条 GATT
客户端连接，不需要断开。

授时后，固件比较本次时间和断电前最后一次保存时间：间隔不超过
`TRIP GAP` 设置值时合并为同一行程，否则结束旧行程并开始新行程。
断电间隔仅用于判定，不计入行驶时间。可选值为 15、30、60、120、
240、480 分钟，默认值为 240 分钟。

完整的行程元数据、游标读取、CRC 和 ACK 格式见
[TRIP_BLE_SYNC.md](TRIP_BLE_SYNC.md)。手机必须先把记录持久化到本地数据库，
再发送累计 ACK；这样中途断线时可以安全续传。
