...
def mode01_demo_bytes(pid):
    """Return realistic core gauge values when the emulator is not in probe mode."""
    t = time.time() - START
    rpm = max(780, int(2900 + 1700 * math.sin(t * 0.82)))
    speed = max(0, int(68 + 26 * math.sin(t * 0.31)))
    load = max(5, min(95, int(48 + 30 * math.sin(t * 0.82))))
    if pid == 0x0C:
        raw = rpm * 4
    ...
    if pid == 0x04:
        return [load * 255 // 100]
    if pid == 0x11:
        return [max(0, min(255, (load - 8) * 3))]
    if pid == 0x10:
        maf_x100 = max(220, min(12000, 180 + rpm * load // 115))
        return [(maf_x100 >> 8) & 0xFF, maf_x100 & 0xFF]
    if pid == 0x42:
        return [0x35, 0xE8]  # 13.8 V
    if pid == 0x44:
        return [0x80, 0x00]  # lambda 1.0 / AFR 14.7
    return None
...
def build_obd_response(sess, req, cfg):
    ...
    if mode == 0x01 and len(data) >= 2:
        ...
        for pid in data[1:]:
            ...
            demo = mode01_demo_bytes(pid) if cfg.probe == "off" else None
            out += [0x41, pid] + (demo if demo is not None else
                                  probe_bytes(cfg, sess.header, single_req, n, pid))
        return out
